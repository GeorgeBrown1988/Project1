...
public:
	//连接数据库
	bool __cdecl ConnectDataBase();
	//关闭连接
	void __cdecl CloseDataBase(bool);
	//设置连接信息
	void __cdecl SettingConnectInfo(CString & szIP,UINT uPort,CString & szUser,CString & szPass,CString & szDB);
	//是否连接错误
	bool __cdecl IsConnectError(); 
	//

	//
	int __cdecl GetDataBaseType(){return m_eType;};
	//获取错误信息
	CString __cdecl GetErrorInfo();
	//清除连接信息
	void __cdecl ClearConnectInfo();
	//
	void __cdecl ClearResult(){;}
	//设置语言编码
	void __cdecl SetLanguage(CString &);
	//
	bool __cdecl SetAutoCommit(bool );
	//
	void __cdecl SetConf(CString &,bool );
	//
	bool __cdecl OnCommit();
//外部功能
public:
	//执行存储过程
	int __cdecl	SQLProc(CString & );
	//生成结果
	bool __cdecl GetFieldValue(int row,int col,CString &);
	//
	bool __cdecl GetFieldValue(int row,int col ,int &);
	//
	bool __cdecl GetFieldValue(int row,int col ,float &);
	//
	//bool __cdecl GetFieldValue(int row,int col,BYTE *pbuf,int size);
	//获取结果
	void __cdecl GetResult(int &row,int &col);
	//释放结果集
	void __cdecl FreeResult();
	//
	bool __cdecl GetStatus(){return m_bStatus;};
	//
...
public:
	...
	//获取接口
	virtual void * __cdecl GetQueueService();
