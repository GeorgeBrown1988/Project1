...
bool __cdecl CTCPSocketService::SetSocketServiceSink(CObject * pIObject)
{
	...
	//设置接口
	IDataQueueSink *pDataQueueSink = dynamic_cast<IDataQueueSink*>(pIObject);
	if(!pDataQueueSink)
	{
		#ifdef _DEBUG
			LOG(INFO) <<"数据对列钩子获取失败";
	...
	if (m_MsgTriggerEvent.SetQueueService(pDataQueueSink)==false)
	{
		#ifdef _DEBUG
...
//设置端口
bool __cdecl CTCPSocketService::SetServicePort(INT wListenPort)
{
	//效验状态
	if (m_bService==true) 
	{
