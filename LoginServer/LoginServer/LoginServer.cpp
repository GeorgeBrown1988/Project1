...
//
HINSTANCE AppInstance;								// current instance
HWND g_hMainWndHwnd;
HWND g_hListWndHwnd;
NOTIFYICONDATA nid;
INT g_uLoginServerPort;
INT g_uDataBasePort;
INT g_uConnectMax;
...
void InitTrayIcon(HWND hwnd)
{
			...
			nid.hIcon = LoadIcon(AppInstance, MAKEINTRESOURCE(IDI_LOGINSERVER));// IMAGE_ICON, 16, 16, 0);
			strcpy(nid.szTip, TEXT("登陆服务器"));
...
void InitDataToControl()
{
	...
	SetDlgItemInt(::g_hMainWndHwnd,IDC_LISTENPORT,::g_uLoginServerPort,false);
	SetDlgItemText(::g_hMainWndHwnd,IDC_DBNAME,::g_szDataBaseName);
	SetDlgItemText(::g_hMainWndHwnd,IDC_DBUSER,::g_szDataBaseUser);
	SetDlgItemInt(::g_hMainWndHwnd,IDC_CONNECTMAX,::g_uConnectMax,false);
}
void ReadConfigFile(CString szFileName)
{
	CIniConfig IniConfig;
	IniConfig.GetConfigNum(szFileName,"LoginServer","LoginServerPort",::g_uLoginServerPort);
	IniConfig.GetConfigNum(szFileName,"LoginServer","DataBasePort",::g_uDataBasePort);
	IniConfig.GetConfigString(szFileName,"LoginServer","DataBaseName",::g_szDataBaseName);
	IniConfig.GetConfigString(szFileName,"LoginServer","DataBaseUser",::g_szDataBaseUser);
	IniConfig.GetConfigString(szFileName,"LoginServer","DataBasePass",::g_szDataBasePass);
	IniConfig.GetConfigString(szFileName,"LoginServer","DataBaseAddr",::g_szDataBaseAddr);
	IniConfig.GetConfigNum(szFileName,"LoginServer","ConnectMax",::g_uConnectMax);
}
void ExitProcessEx()
{
...
INT_PTR CALLBACK MainWndProc(HWND hMainWndHwnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	switch(uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			InitControl();
			TraceEventString("登陆服务器运行",Level_Normal);
			return true;
		}
		case WM_ICONNOTIFY:
		{
		...
		case WM_COMMAND:
		{
			switch(wParam)
			{
				...
				case IDC_EXIT:
				{
					if(1)//if(g_CenterService.IsService())
					{
                     ...
                     int       nCmdShow)
{
	...
	DialogBox(AppInstance, MAKEINTRESOURCE(IDD_LOGINDLG),NULL, MainWndProc);
}
